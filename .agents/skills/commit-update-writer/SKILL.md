...
description: 基于指定 CAM-FE 改动范围分析 federation remote、平台合同、请求层与业务界面链路，并将工程记录增量写入 docs/COMMITLOG.md。仅在用户明确要求改动记录、commit update 或 commitlog 时使用。
---

# Commit Update Writer

此 skill 记录 CAM-FE 的工程改动，不是复述 diff 或产品公告。CAM-FE 是 CDI-Pedestal 加载的 `cam/App` federation remote：写作必须说明 host 合同、请求认证、服务调用、业务交互、i18n 和 remote 产物的真实上下游影响。
...
除 hunks 外，补读直接上游和下游：CDI-Pedestal 的 `PlatformContextValue` 与 remote 装载、`src/remote.tsx`、`setPlatformAuth`/`setApiBase`、Axios interceptors 与 `ApiError`、服务层/组件/hook、`/api` 代理、Vite public path、manifest、`remoteEntry.js`、Caddy、locale 和运行入口。

重点还原：改动前行为；所解决的问题和约束；改动后的实现与取舍；对 host、请求认证、业务数据、组件状态、翻译和部署资产的影响。不要把真实后端未验证、测试账号未授权或构建未执行写成已完成事实。

## 执行步骤

1. 判定范围和基线，收集文件级/hunk 级 diff（新增、修改、删除、重命名）。
2. 为关键改动构建“上游依赖 → 当前改动 → 下游影响”链路，并检查 host、服务和 UI 消费者是否同步。
...
- 上游依赖：<host 合同、request、服务或构建配置>
- 当前改动：<真实模块、行为与取舍>
- 下游影响：<Pedestal、组件、业务接口、翻译或部署消费者>
